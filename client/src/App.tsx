...
import { getLastProject, getRecentProjects, saveLastProject, loadLlmConfig, loadLanguage, loadSearchApiConfig, loadEmbeddingConfig, loadMultimodalConfig, loadOutputLanguage, loadProviderConfigs, loadActivePresetId, loadProxyConfig, loadChatSystemPrompt } from "@/lib/project-store"
...
function App() {
  ...
  const [authToken, setAuthToken] = useState<string | null>(() => localStorage.getItem("kos_auth_token"))
  const [currentUser, setCurrentUser] = useState<any | null>(() => {
    const saved = localStorage.getItem("kos_current_user")
  ...
  if (!authToken || !currentUser) {
    return (
      <LoginScreen onLogin={(token, user) => {
        setAuthToken(token)
        setCurrentUser(user)
