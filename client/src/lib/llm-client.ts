...
export async function streamChat(
  ...
  // providerConfig is now only used for fallback/metadata if any,
  // we route the actual request through our backend proxy!
  ...
  try {
    ...
    const serverUrl = store.serverConfig?.url || "http://127.0.0.1:8080"
    
    // Default to the standard completions proxy unless explicitly requested to use the RAG pipeline
    const isRag = (requestOverrides as any)?.isRag === true
    const proxyUrl = isRag 
      ? `${serverUrl}/api/v1/chat/rag` 
      : `${serverUrl}/api/v1/chat/completions`
    ...
    // Remove custom internal flags before sending to LiteLLM
    delete (requestBody as any).isRag

    const { getAuthToken, getProjectServerId } = await import("@/lib/api-client")
    ...
    response = await httpFetch(proxyUrl, {
      method: "POST",
      headers,
    ...
    if (isFetchNetworkError(err)) {
      ...
      onError(new Error(`Network error reaching the backend proxy. Check endpoint URL, API key, and connectivity.`))
      return
    }
  ...
  // Parse standard OpenAI SSE chunk which LiteLLM returns
  const parseProxyStream = (text: string): string | null => {
    if (text === "data: [DONE]") return null
    if (!text.startsWith("data: ")) return null
    try {
      const payload = JSON.parse(text.slice(6))
      
      // Handle custom refs event
      if (payload.type === "refs") {
        if (callbacks.onRefs && Array.isArray(payload.refs)) {
          callbacks.onRefs(payload.refs)
        }
        return null
      }
    ...
    while (true) {
      ...
      if (done) {
        if (lineBuffer.trim()) {
          ...
          const token = parseProxyStream(lineBuffer.trim())
      ...
      for (const line of lines) {
        ...
        const token = parseProxyStream(trimmed)
        if (token !== null) recordToken(token)
      }
    }
