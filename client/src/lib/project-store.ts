...
const CHAT_SYSTEM_PROMPT_KEY = "chatSystemPromptOverride"

export async function saveChatSystemPrompt(prompt: string): Promise<void> {
  const store = await getStore()
  await store.set(CHAT_SYSTEM_PROMPT_KEY, prompt)
}

export async function loadChatSystemPrompt(): Promise<string | null> {
  const store = await getStore()
  return (await store.get<string>(CHAT_SYSTEM_PROMPT_KEY)) ?? null
}
