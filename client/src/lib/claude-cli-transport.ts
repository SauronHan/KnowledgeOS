 ...
 *      otherwise mis-diagnose it as an Lyrebird KOS bug.
...
export function buildExitError(
  ...
  if (/unauthenticated|please.*log\s*in|authentication.*failed/i.test(stderr)) {
    return [
      ...
      "then retry. (Lyrebird KOS only spawns the binary — it can't run the",
  ...
  if (unparsedStdout.trim()) {
    return [
      ...
      "Captured stdout output that Lyrebird KOS couldn't parse — pasting it",
