...
import { queueResearch, publishResearchToWiki, loadPreviousResearch } from "@/lib/deep-research"
import { normalizePath } from "@/lib/path-utils"
import { kosReadFile } from "@/lib/api-client"
...
export function ResearchPanel() {
  ...
  useEffect(() => {
    if (project?.path) {
      loadPreviousResearch(project.path)
    }
  }, [project?.path])
  ...
  function handleStartResearch() {
    ...
    queueResearch(normalizePath(project.path), topic, llmConfig)
    setInputValue("")
  }
...
function SynthesisBlock({ synthesis, isStreaming, editable, onChange }: { 
  synthesis: string; 
  isStreaming: boolean;
  editable?: boolean;
      ...
      >
        {editable ? (
          <textarea
            ...
            className="w-full h-full min-h-[200px] bg-transparent border-none outline-none text-[11px] font-mono resize-none"
            placeholder="Edit your research result..."
          />
        ) : (
          <>
            {thinking && (
...
function ResearchTaskCard({ task, onRemove }: { task: ResearchTask; onRemove: (id: string) => void }) {
  ...
  async function handleOpenSaved() {
    ...
    try {
      // In this new workflow, savedPath is a local absolute path for the draft.
      const content = await readFile(task.savedPath)
      // Since it's a local draft, we just view it in the store, 
      // not yet as a server document.
      setSelectedFile(task.savedPath)
      setFileContent(content)
    } catch (err) {
      console.error("Failed to read local research draft:", err)
      window.alert("Could not open local file. It might have been moved or deleted.")
    }
  }
  ...
  async function handleSaveToWiki() {
    if (!project) return
    try {
      await publishResearchToWiki(task.id)
      window.alert("Research result has been published to your Wiki! It's now being processed in the cloud.")
    } catch (err) {
      ...
      {expanded && (
        <div className="border-t px-3 py-2">
          ...
          <div className="flex items-center gap-1.5 mt-2">
            ...
            {task.status === "done" && (
              <Button variant="default" size="sm" className="h-6 text-[11px] gap-1 bg-blue-600 hover:bg-blue-700" onClick={handleSaveToWiki}>
                <FileText className="h-3 w-3" />
                Save to Wiki
              </Button>
            )}
