...
export function useSourceFiles() {
  ...
  return sources.length > 0 ? sources : cachedSourceFiles
}
...
function SaveToWikiButton({ content, visible }: { content: string; visible: boolean }) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${serverUrl}/api/v1/chat/save`, {
        method: "POST",
        headers: {
      ...
      // Optionally refresh UI state if needed, e.g., triggering a refresh of the sources view
    } catch (err) {
        ...
        <ReactMarkdown
          ...
          components={{
            a: ({ href, children }) => {
              ...
              // If it's a local file citation (e.g. wiki/..., document_...), render as plain text [1] without link styles
              if (href && (href.startsWith("wiki/") || href.startsWith("document_") || href.includes(".md"))) {
                return (
                  <span className="text-muted-foreground text-[10px] bg-muted px-1 py-0.5 rounded ml-1 cursor-default" title="Citation">
                    {children}
                  </span>
                )
              }
              // External links
              return (
                <a href={href} target="_blank" rel="noreferrer" className="text-primary underline break-all" title={href}>
                  {children}
                </a>
              )
            },
